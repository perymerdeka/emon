...
async def user1_trans_headers(client: TestClient) -> dict: # Make async
    ...
    await client.post("/auth/register", json={"email": user1_email, "password": user1_pw}) # Use await
...
async def user2_trans_headers(client: TestClient) -> dict: # Make async
    ...
    await client.post("/auth/register", json={"email": user2_email, "password": user2_pw}) # Use await
...
async def user1_categories(client: TestClient, user1_trans_headers: dict) -> dict: # Make async
    ...
    resp_income = await client.post("/categories/", headers=user1_trans_headers, json=income_data) # Use await
    resp_expense = await client.post("/categories/", headers=user1_trans_headers, json=expense_data) # Use await
...
async def test_create_transaction_unauthenticated(client: TestClient): # Mark async
    ...
    response = await client.post("/transactions/", json=trans_data) # Use await
...
async def test_create_transaction_success(client: TestClient, user1_trans_headers: dict, user1_categories: dict): # Mark async
    ...
    response = await client.post("/transactions/", headers=user1_trans_headers, json=trans_data) # Use await
...
async def test_create_transaction_wrong_user_category( # Mark async
    ...
    resp_cat = await client.post("/categories/", headers=user2_trans_headers, json=cat_data) # Use await
...
async def test_read_transactions_authenticated_empty(client: TestClient, user1_trans_headers: dict): # Mark async
    ...
    response = await client.get("/transactions/", headers=user1_trans_headers) # Use await
...
async def test_read_transactions_filtering_and_ownership( # Mark async
    ...
    await client.post("/transactions/", headers=user1_trans_headers, json=t1_data) # Use await
    await client.post("/transactions/", headers=user1_trans_headers, json=t2_data) # Use await
    await client.post("/transactions/", headers=user1_trans_headers, json=t3_data) # Use await

    # User 2 transaction (create category first)
    cat_data_u2 = {"name": "User2 Income", "type": CategoryType.INCOME}
    resp_cat_u2 = await client.post("/categories/", headers=user2_trans_headers, json=cat_data_u2) # Use await
    user2_cat_id = resp_cat_u2.json()["id"]
    t4_data = {"amount": 2000, "date": str(today), "category_id": user2_cat_id}
    await client.post("/transactions/", headers=user2_trans_headers, json=t4_data) # Use await

    # Read all for User 1
    resp_all_u1 = await client.get("/transactions/", headers=user1_trans_headers) # Use await
    ...
    resp_date_u1 = await client.get(f"/transactions/?start_date={today}&end_date={today}", headers=user1_trans_headers) # Use await
    assert resp_date_u1.status_code == 200
    assert len(resp_date_u1.json()) == 2

    # Read filtered by category for User 1
    resp_cat_u1 = await client.get(f"/transactions/?category_id={user1_categories['expense_id']}", headers=user1_trans_headers) # Use await
    assert resp_cat_u1.status_code == 200
    assert len(resp_cat_u1.json()) == 2

    # Read all for User 2
    resp_all_u2 = await client.get("/transactions/", headers=user2_trans_headers) # Use await
...
async def test_read_transaction_by_id_wrong_user( # Mark async
    ...
    resp_create = await client.post("/transactions/", headers=user1_trans_headers, json=trans_data) # Use await
    trans_id = resp_create.json()["id"]

    # User 2 tries to read User 1's transaction
    resp_read = await client.get(f"/transactions/{trans_id}", headers=user2_trans_headers) # Use await
...
async def test_update_transaction(client: TestClient, user1_trans_headers: dict, user1_categories: dict, session: DbSession): # Mark async, use DbSession
    ...
    resp_update = await client.put(f"/transactions/{trans_id}", headers=user1_trans_headers, json=update_data) # Use await
...
async def test_update_transaction_wrong_user( # Mark async
    ...
    resp_update = await client.put(f"/transactions/{trans_id}", headers=user2_trans_headers, json=update_data) # Use await
...
async def test_delete_transaction(client: TestClient, user1_trans_headers: dict, user1_categories: dict, session: DbSession): # Mark async, use DbSession
    ...
    # Delete
    resp_delete = await client.delete(f"/transactions/{trans_id}", headers=user1_trans_headers) # Use await
...
async def test_delete_transaction_wrong_user( # Mark async
    ...
    resp_delete = await client.delete(f"/transactions/{trans_id}", headers=user2_trans_headers) # Use await
